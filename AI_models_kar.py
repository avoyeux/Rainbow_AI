...
class VaryingLSTMKernelSizes:
    ...
    def __init__(self, sequence_len: int = 8, image_size: int = 512, kernel_size_conv: list = [(3, 3), (5, 5), (7, 7)],
                 kernel_size_LSTM: list = [(9, 9), (5, 5), (3, 3)]):
...
class SetLSTMKernelSizes:
    """
    WORK IN PROGRESS
    Trying to see if I can improve the last architecture a little given my new understanding of the model.
    """

    def __init__(self, sequence_len: int = 8, image_size: int = 512, kernel_size_conv: tuple = (3, 3),
                 kernel_size_LSTM: tuple = (5, 5)):
    ...
    def UNet(self):
        ...
        p0 = inputs
        c1, p1 = self.Down_block(p0, f[0], kernel_size=self.kernel_size_LSTM, first=True)
        c2, p2 = self.Down_block(p1, f[1], kernel_size=self.kernel_size_LSTM)
        c3, p3 = self.Down_block(p2, f[2], kernel_size=self.kernel_size_LSTM)

        bn = self.Bottleneck(p3, f[3], kernel_size=(3, 3))

        u0 = self.Up_block(bn, c3, f[2], kernel_size=self.kernel_size_conv)
        u1 = self.Up_block(u0, c2, f[1], kernel_size=self.kernel_size_conv)
        u2 = self.Up_block(u1, c1, f[0], kernel_size=self.kernel_size_conv)
