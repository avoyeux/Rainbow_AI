...
class ImprovedUNetConvLSTM2D:
    """
    WORK IN PROGRESS
    Trying to see if I can improve the last architecture a little given my new understanding of the model.
    """

    def __init__(self, sequence_len: int = 8, image_size: int = 512, kernel_size: tuple = (5, 5)):
    ...
    def Down_block(self, x, filters: int, kernel_size: tuple, padding='same', strides=1, first=False):
        """
        To downscale the input and use an increasing number of filters.
        """
    ...
    def UNet(self):
        ...
        p0 = inputs
        c1, p1 = self.Down_block(p0, f[0], kernel_size=(9, 9), first=True)
        c2, p2 = self.Down_block(p1, f[1], kernel_size=(5, 5))
        c3, p3 = self.Down_block(p2, f[2], kernel_size=(3, 3))

        bn = self.Bottleneck(p3, f[3])

        u0 = self.Up_block(bn, c3, f[2])
        u1 = self.Up_block(u0, c2, f[1])
        u2 = self.Up_block(u1, c1, f[0])
...
class ImprovedUNetConvLSTM2D_GPU_test:
    """
    WORK IN PROGRESS
    Trying to see if I can improve the last architecture a little given my new understanding of the model.
    """
